...
struct Fourier{TT,SType}
    ...
    modes::Int
    Fourier(stype, amplitudes, m) = new{Float64,stype}(stype, amplitudes, m)
end

const FourierCosineSeries{TT} = FourierSeries{TT,:cos}
const FourierSineSeries{TT} = FourierSeries{TT,:sin}
...
struct FourierSeries{TT,ST,CT}
    ...
    function FourierSeries(series...)
        ...
        new{TT,typeof(cosine),typeof(sine)}(cosine, sine)
    end
end
